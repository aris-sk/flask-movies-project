...
## Custom Validator εκτός κλάσεων φορμών για έλεγχο του μεγέθους των εικόνων που δίνονται
...
## Validator εκτός της κλάσης SignupForm, για έλεγχο ύπαρξης του email στη βάση 
...
class AccountUpdateForm(FlaskForm):
    ...
    image = FileField('Εικόνα προφίλ', validators=[Optional(strip_whitespace=True),
                                    FileAllowed(['jpg', 'jpeg', 'png'], 'Αποδεκτοί τύποι εικόνων: jpg, jpeg και png!'),
...
class NewMovieForm(FlaskForm):
    ...
    plot = TextAreaField(label="Υπόθεση Ταινίας",
                               validators=[DataRequired(message="Αυτό το πεδίο δε μπορεί να είναι κενό."), 
                                           Length(min=5, message="Το κείμενο της υπόθεσης της ταινίας πρέπει να έχει τουλάχιστον 5 χαρακτήρες")])

    ## Αρχείο Εικόνας, με επιτρεπόμενους τύπους εικόνων τα 'jpg', 'jpeg', 'png',
    ## και μέγιστο μέγεθος αρχείου εικόνας τα 2 MBytes, ΜΗ υποχρεωτικό πεδίο
    image = FileField('Εικόνα Ταινίας', validators=[Optional(strip_whitespace=True),
                                                               FileAllowed([ 'jpg', 'jpeg', 'png' ],
                                                                'Αποδεκτοί τύποι εικόνων: jpg, jpeg και png!'),
                                                               maxImageSize(max_size=2)])

    ## IntegerField με το έτος πρώτης προβολής της ταινίας, θα παίρνει τιμές από το 1888 έως το current_year
    ## που υπολογίζεται στην αρχή του κώδικα εδώ στο forms.py
    release_year = IntegerField(label="Έτος πρώτης προβολής",
                                   validators=[NumberRange(min=1888, max=current_year, message=f"Το έτος πρώτης προβολής της ταινίας, παίρνει τιμές από το 1888 έως το τρέχων έτος ({current_year})")]) 
    ...
    rating = IntegerField(label="Βαθμολογία Ταινίας",
                                       validators=[DataRequired(message="Αυτό το πεδίο δε μπορεί να είναι κενό."),
                                                   NumberRange(min=1, max=100, message=f"Η βαθμολογία ταινίας πρέπει να είναι από 1 έως 100")])
